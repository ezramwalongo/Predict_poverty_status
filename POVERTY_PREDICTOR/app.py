...
<style>
    ...
    /* Simplified background without CPU-intensive animation */
    .stApp {{
        background-image: linear-gradient(160deg, #e0f2fe 0%, #bae6fd 55%, #7dd3fc 100%);
        background-size: cover;
        background-attachment: fixed;
    }}

    /* Static Title for better performance */
    .animated-title {{
    ...
    /* High-contrast, bold text everywhere visible in the app */
    ...
    /* Make sure all text inside tables and dataframes is also black and bold */
...
@st.cache_data
def get_cached_regions(lang):
    return get_regions(lang)

@st.cache_data
def get_cached_districts(region, lang):
    return get_districts(region, lang)
...
def prediction_form():
    ...
    if step == 0:
        ...
        # Get regions (cached)
        regions = get_cached_regions(st.session_state.language)
        ...
        draft['district'] = 'All' # Set a default for storage consistency
    ...
    with nav3:
        if step < len(STEP_KEYS) - 1:
            ...
            if st.button(t('predict_button'), use_container_width=True, type="primary", key='predict_btn'):
                ...
                st.session_state.show_results = True  # Show results immediately
                st.rerun()
...
def display_results():
    ...
    with col1:
        # Show probability table instead of gauge chart
        prob = result['probability']
        prob_percent = prob * 100
        
        # Define categories based on probability
        if prob >= 0.8:
            status = "Poorer"
        elif prob >= 0.5:
            status = "Poor"
        ...
        prob_data = {
            "Category": ["Status", "Probability"],
            "Value": [status, f"{prob_percent:.1f}%"]
        }
    ...
    # Always display the chart (already handled by display_results being called)
    fig = px.bar(
        importance_df,
        x=t('contribution'),
        y=t('factor'),
...
def research_dashboard():
    ...
    with col2:
        # Small table for poor and non-poor percent
        poor_pct = stats['poor_percentage']
        non_poor_pct = 100 - poor_pct
        
        stats_data = {
            "Poverty Status": [t('poor'), t('non_poor')],
            "Percentage": [f"{poor_pct:.1f}%", f"{non_poor_pct:.1f}%"]
        }
    ...
    with col1:
        regions = [t('filter_all')] + get_cached_regions(st.session_state.language)
