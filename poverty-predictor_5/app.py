...
<style>
    ...
    .stApp {{
        background-image: url("{_BG_SVG_URI}"), linear-gradient(160deg, #0c0a24 0%, #14123a 55%, #1b1745 100%);
    ...
    .animated-title {{
        ...
        text-shadow: 0 2px 10px rgba(99,102,241,0.6), 0 1px 3px rgba(0,0,0,0.7);
...
def build_results_table(result: Dict) -> pd.DataFrame:
    """Build the poverty-status results table: every status row is shown,
    with its probability-distribution range (in 0-1 form, not %), and a
    tick (matching row) / cross (other rows) mark based on which status the
    household's predicted probability falls into."""
    rows = []
    for tier_key, range_str in STATUS_RANGES:
        rows.append({
            t('poverty_status'): t(f'tier_{tier_key}'),
            t('probability_distribution'): range_str,
            t('probability_mark'): '✅' if tier_key == result['tier'] else '❌',
        })
