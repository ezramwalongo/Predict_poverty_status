...
- Python 3.11 — set this explicitly via **Advanced settings → Python version** when you deploy on Streamlit Community Cloud (see Deployment section below). A `runtime.txt` is included as a best-effort hint, but Community Cloud frequently ignores it, so Advanced Settings is the reliable method.
- Streamlit 1.38.0+
- pandas, numpy, scikit-learn, plotly

> **Deploying to Streamlit Community Cloud and seeing "Error installing requirements" or other errors?**
> Click **Manage app → terminal** to see which exact package failed — that tells you the real cause. Common causes: (1) Python version mismatch — Community Cloud defaults to 3.12 (and has been seen forcing 3.13/3.14) regardless of `runtime.txt`, so explicitly set **Python 3.11** in **Advanced settings** when deploying (you can't change this after deploying — delete and redeploy instead); (2) an exact `==` pin with no wheel for the resolved Python version, or two pins that can't be resolved together — `requirements.txt` here uses minimum-version (`>=`) pins for this reason. If it still fails, paste the exact terminal error and I can pinpoint the fix.
...
6. **Before clicking Deploy**, open **"Advanced settings"** and set **Python version to 3.11** from the dropdown. This is the reliable way to pin the Python version — `runtime.txt` is frequently ignored by Streamlit Community Cloud (a known platform issue), so don't rely on it alone.
7. Deploy

> **If you already deployed and it's misbehaving:** you cannot change an existing app's Python version — delete the app and redeploy it, setting Python 3.11 in Advanced Settings this time.
...
### Registered users can log in during a session, but "disappear" later / registration doesn't stick

This app stores accounts (`data/users.csv`) and prediction history (`data/predictions.csv`) as plain CSV files on disk. That works perfectly for local use and for Docker deployments with a persistent volume — but on **Streamlit Community Cloud's free tier, the filesystem is ephemeral**: any file written while the app is running is wiped whenever the app restarts, redeploys, or wakes up from going to sleep after inactivity. So a user can register and use the app fine in one session, but if the app restarts before they come back, their account is gone and login will (correctly) say "no account found" — this is expected behavior for CSV storage on that hosting tier, not a bug in the login logic itself.

If you need accounts/predictions to survive restarts on Streamlit Community Cloud, you have two options:
1. **Deploy somewhere with a persistent disk instead** (a small VM, Render, Railway, Fly.io, or the included `Dockerfile` with a mounted volume) — no code changes needed.
2. **Swap the CSV storage for an external store** (e.g., a free-tier Postgres/SQLite-on-a-volume, Google Sheets, or Streamlit's `st.connection` to a database) — this does need code changes in `utils/auth.py` and `utils/storage.py`; ask if you'd like this implemented.

### "Error installing requirements" / app fails to start on Streamlit Community Cloud

See the note under **Requirements** above — this is almost always a Python-version mismatch. Set **Python 3.11** explicitly in **Advanced settings** when deploying (`runtime.txt` alone is not reliable). If the error persists, copy the exact text from **Manage app → terminal** — the real cause is always shown there, one line above the generic "Error installing requirements" message.
...
- Hardened `language_toggle()` against Streamlit API version mismatches (falls back gracefully instead of crashing the app).
- Documented the real fix for Streamlit Community Cloud deploy failures: set Python 3.11 via **Advanced settings** (not just `runtime.txt`, which the platform frequently ignores).
- Documented the ephemeral-storage limitation of Streamlit Community Cloud's free tier (registered users / predictions don't survive app restarts there) and how to get real persistence.
