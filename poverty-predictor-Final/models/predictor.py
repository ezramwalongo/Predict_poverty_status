    ...
    if os.path.exists(COEFFICIENTS_PATH):
        try:
            ...
            return (
                data['intercept'],
                data['coefficients'],
                data.get('trained', True),
                data.get('train_accuracy'),
                data.get('test_accuracy'),
            )
        except Exception:
            pass
    return FALLBACK_INTERCEPT, dict(FALLBACK_COEFFICIENTS), False, None, None
...
class PovertyPredictor:
    ...
    def __init__(self):
        (self.intercept, self.coefficients, self.is_trained,
         self.train_accuracy, self.test_accuracy) = _load_coefficients()
    ...
    def predict(self, features: Dict[str, float]) -> Dict:
        ...
        Returns a dict with:
            ...
            - tier: poverty-status class ('poor' or 'non_poor')
    ...
    def _tier_for(probability: float) -> str:
        """
        Two-class poverty-status classification matching the results table:
          Poor      : 0.60 - 1.00
          Non Poor  : 0.00 - 0.50

        The table leaves 0.51-0.59 undefined; to guarantee every probability
        maps to exactly one row (no gaps), that band is folded into "Non
        Poor" here (i.e. Non Poor effectively covers [0.00, 0.60)). The
        table displayed to the user still shows the original 0.00-0.50
        range as given.
        """
        if probability >= 0.6:
            return 'poor'
