...
def build_results_table(result: Dict) -> pd.DataFrame:
    """Build the results summary as a table (status + probability), used
    instead of the previous gauge chart. No negative delta is shown — only
    the plain poverty probability and a descriptive status."""
    status_label = t(f"tier_{result['tier']}")
    return pd.DataFrame([{
        t('result_status'): status_label,
        t('result_probability'): result['score'],
    }])
...
def display_results():
    ...
    # Results table (replaces the previous gauge chart). Shows a descriptive
    # status (Very Poor / Poor / Non-poor / Well-off) and the plain poverty
    # probability — no negative delta percentage is shown.
    ...
    with col2:
        st.metric(t('score'), result['score'])
